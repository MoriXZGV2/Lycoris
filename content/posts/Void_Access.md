+++
title = "System Log: Unauthorized Access"
date = 2026-08-08
draft = false
description = "Anomaly detected in sector 7."
[taxonomies]
tags = ["Rahasia", "Base64"]
+++

{% terminal(file="access.log", track="static.ogg", dur="0:30", durS="30.0", trackDesc="buat kamu yang lagi ngerasa sendiri.", design="3", show="2") %}
--- access.log

--- output.log
Base64 content loaded. Decode to reveal the hidden transmission.
Stego image embedded: udang_stego.webp
{% end %}

![Stego Image](/images/udang_stego.webp)